...
func (repository *mySQLRepository) GetStateSummaryCumulative(state string) ([]Entry, error) {
	var result []Entry
	stateQuery := fmt.Sprintf("select * from %s where state='%s'", repository.table, state)
	query := fmt.Sprintf("SELECT * FROM (SELECT a.date, a.country, a.state, sum(b.infected), sum(b.hospitalized), sum(b.critical), sum(b.dead), sum(b.recovered), sum(b.active) AS cumulative FROM (%s) a JOIN (%s) b ON a.date >= b.date GROUP BY date,country) AS total WHERE date='2020-04-13';", stateQuery, stateQuery)
	queryResult, err := repository.client.Query(query)
	if err != nil {
		return nil, err
	}
...
func (repository *mySQLRepository) GetCountrySummaryCumulative() ([]Entry, error) {
	var result []Entry
	countryQuery := fmt.Sprintf("select date, country, sum(infected) as infected, sum(hospitalized) as hospitalized, sum(critical) as critical, sum(dead) as dead, sum(recovered) as recovered, sum(active) as active from %s group by date,country", repository.table)
	query := fmt.Sprintf("SELECT * FROM (SELECT a.date, a.country, sum(b.infected), sum(b.hospitalized), sum(b.critical), sum(b.dead), sum(b.recovered), sum(b.active) AS cumulative FROM (%s) a JOIN (%s) b ON a.date >= b.date GROUP BY date,country) AS total WHERE date='2020-04-13';", countryQuery, countryQuery)
	fmt.Println(query)
	queryResult, err := repository.client.Query(query)
	if err != nil {
		return nil, err
...
func (repository *mySQLRepository) GetStateSummaryDeltas(state string) ([]Entry, error) {
	var result []Entry
	query := fmt.Sprintf("SELECT date, country, state, infected, hospitalized, critical, dead, recovered, active FROM %s WHERE state='%s' AND date='2020-04-13';", repository.table, state)
	queryResult, err := repository.client.Query(query)
	if err != nil {
		return nil, err
	}
...
func (repository *mySQLRepository) GetCountrySummaryDeltas() ([]Entry, error) {
	var result []Entry
	query := fmt.Sprintf("select date, country, sum(infected), sum(hospitalized), sum(critical), sum(dead), sum(recovered), sum(active) from %s WHERE date='2020-04-13' GROUP BY country;", repository.table)
	queryResult, err := repository.client.Query(query)
	if err != nil {
		return nil, err
	}
